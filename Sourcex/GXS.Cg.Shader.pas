...
type
  ...
  TCGxProgram = class(TGSUpdateAbleObject)
  private
...
procedure TCGxParameter.SetParameterPointer(Values: TGSAffineVectorList);
begin
  Assert(FVariability = CG_VARYING);
  cgGLSetParameterPointer(FHandle, 3, GL_FLOAT, 0, Values.List);
end;

procedure TCGxParameter.SetParameterPointer(Values: TGSVectorList);
begin
  Assert(FVariability = CG_VARYING);
