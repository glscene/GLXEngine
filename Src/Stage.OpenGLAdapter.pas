...
{$IFDEF USE_LOGGING}
  if length > 0 then
    GSLogger.LogDebug(string(message));
{$ENDIF}
end;
...
begin
{$IFDEF USE_LOGGING}
  GSLogger.LogError('Call OpenGL function with undefined entry point');
{$ENDIF}
  Abort;
end;
...
{$IFDEF USE_OPENGL_DEBUG}
  if Result <> @glCap then
    GSLogger.LogDebug('Finded entry point of ' + vName)
  else
    GSLogger.LogDebug('Can''t find entry point of ' + vName);
{$ENDIF}
end;
    ...
    try
      ...
      begin
        ...
        if not(FDebug and ARB_debug_output) then
          case glError of
            GL_INVALID_ENUM:
              GSLogger.LogError(format(strOpenGLError, ['Invalid enum']));
            GL_INVALID_VALUE:
              GSLogger.LogError(format(strOpenGLError, ['Invalid value']));
            GL_INVALID_OPERATION:
              GSLogger.LogError(format(strOpenGLError,
                ['Invalid Operation']));
            GL_OUT_OF_MEMORY:
              GSLogger.LogError(format(strOpenGLError, ['Out of memory']));
          end;
      end;
    except
      GSLogger.LogError(format(strOpenGLError, ['Exception in glGetError']));
    end;
end;
...
begin
  GSLogger.LogNotice('Getting OpenGL entry points and extension');
{$IFDEF SUPPORT_WGL}
  ReadWGLExtensions;
...
{$ENDIF}
  ...
  begin
    GSLogger.LogNotice('');
    GSLogger.LogInfo('OpenGL rendering context information:');
    GSLogger.LogInfo(format('Renderer     : %s', [GetString(GL_RENDERER)]));
    GSLogger.LogInfo(format('Vendor       : %s', [GetString(GL_VENDOR)]));
    GSLogger.LogInfo(format('Version      : %s', [GetString(GL_VERSION)]));
    if VERSION_2_0 then
      GSLogger.LogInfo(format('GLSL version : %s',
        [GetString(GL_SHADING_LANGUAGE_VERSION)]))
    else
      GSLogger.LogWarning('GLSL version : not supported');
    GSLogger.LogNotice('');
    vNotInformed := False;
  end;
