...
procedure SetBB(var C: THmgBoundingBox; const V: TGSVector);
procedure SetAABB(var Bb: TAABB; const V: TGSVector); inline;

procedure BBTransform(var C: THmgBoundingBox; const M: TGSMatrix);
procedure AABBTransform(var Bb: TAABB; const M: TGSMatrix);
...
procedure AABBFromSweep(var SweepAABB: TAABB; const Start, Dest: TGSVector; const Radius: Single);
...
function AABBToBB(const AnAABB: TAABB; const M: TGSMatrix): THmgBoundingBox; overload;
//  Adds delta to min and max of the AABB.
procedure OffsetAABB(var Aabb: TAABB; const Delta: TAffineVector); overload;
procedure OffsetAABB(var Aabb: TAABB; const Delta: TGSVector); overload;
//  Adds delta to min and max of the BB.
procedure OffsetBB(var Bb: THmgBoundingBox; const Delta: TAffineVector); overload;
procedure OffsetBB(var Bb: THmgBoundingBox; const Delta: TGSVector); overload;
// The same as above but uses AddPoint() instead of AddVector().
procedure OffsetBBPoint(var Bb: THmgBoundingBox; const Delta: TGSVector); overload;
...
function IntersectAABBs(const Aabb1, Aabb2: TAABB; const M1To2, M2To1: TGSMatrix): Boolean; overload;
...
function PointInAABB(const P: TGSVector; const Aabb: TAABB): Boolean; overload;
...
function PlaneAABBIntersection(const plane : THmgPlane; const AABB : TAABB) : TGSAffineVectorList;
(*
...
function BSphereToAABB(const Center: TGSVector; Radius: Single): TAABB; overload;
...
procedure IncludeInClipRect(var ClipRect: TGSClipRect; X, Y: Single);
// Projects an AABB and determines the extent of its projection as a clip rect.
function AABBToClipRect(const Aabb: TAABB; const ModelViewProjection: TGSMatrix;
  ViewportSizeX, ViewportSizeY: Integer): TGSClipRect;

// Finds the intersection between a ray and an axis aligned bounding box.
function RayCastAABBIntersect(const RayOrigin, RayDirection: TGSVector;
  const Aabb: TAABB; out TNear, TFar: Single): Boolean; overload;
function RayCastAABBIntersect(const RayOrigin, RayDirection: TGSVector;
  const Aabb: TAABB; IntersectPoint: PGSVector = nil): Boolean; overload;
...
//---------------------------------------------------------------------------
procedure SetAABB(var Bb: TAABB; const V: TGSVector);
begin
  Bb.Max.X := Abs(V.X);
  Bb.Max.Y := Abs(V.Y);
  Bb.Max.Z := Abs(V.Z);
...
//---------------------------------------------------------------------------
procedure AABBFromSweep(var SweepAABB: TAABB; const Start, Dest: TGSVector;
  const Radius: Single);
begin
...
//---------------------------------------------------------------------------
function AABBToBB(const AnAABB: TAABB; const M: TGSMatrix): THmgBoundingBox;
begin
...
//---------------------------------------------------------------------------
procedure OffsetAABB(var Aabb: TAABB; const Delta: TGSVector);
begin
  AddVector(Aabb.Min, Delta);
  AddVector(Aabb.Max, Delta);
end;
...
//---------------------------------------------------------------------------
procedure OffsetBB(var Bb: THmgBoundingBox; const Delta: TGSVector);
var
  I: Integer;
begin
  for I := 0 to 7 do
...
//---------------------------------------------------------------------------
procedure OffsetBBPoint(var Bb: THmgBoundingBox; const Delta: TGSVector);
var
  I: Integer;
begin
  for I := 0 to 7 do
...
function IntersectAABBs(const Aabb1, Aabb2: TAABB;
  const M1To2, M2To1: TGSMatrix): Boolean;
const
...
//---------------------------------------------------------------------------
function PointInAABB(const P: TGSVector; const Aabb: TAABB): Boolean;
begin
  Result := (P.X <= Aabb.Max.X) and
    (P.X >= Aabb.Min.X) and (P.Y <= Aabb.Max.Y) and
    (P.Y >= Aabb.Min.Y) and (P.Z <= Aabb.Max.Z) and
...
//---------------------------------------------------------------------------
function BSphereToAABB(const Center: TGSVector; Radius: Single): TAABB;
begin
...
//---------------------------------------------------------------------------
// see article_frustumculling.shtml
//---------------------------------------------------------------------------
