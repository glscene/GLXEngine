...
******************************************************************************)
unit Stage.Coordinates;
(*
  Coordinate related classes and functions.
  The registered classes are:
    [TGSCoordinates2, TGSCoordinates3, TGSCoordinates4]
*)
interface
...
type
  ...
  (* Identifies the type of data stored within a TGSCustomCoordinates.
     csPoint2D : a simple 2D point (Z=0, W=0)
     csPoint : a point (W=1)
     csVector : a vector (W=0)
     csUnknown : no constraint *)
  TGSCoordinatesStyle = (csPoint2D, csPoint, csVector, csUnknown);

  (* Stores any homogeneous vector.
    This class is basicly a container for a TGSVector, allowing proper use of
  ...
  TGSCustomCoordinates = class(TGSUpdateAbleObject)
  private
    FCoords: TGSVector;
    FStyle: TGSCoordinatesStyle; // NOT Persistent
    FPDefaultCoords: PGSVector;
  ...
  protected
    procedure SetDirectVector(const V: TGSVector); inline;
    procedure DefineProperties(Filer: TFiler); override;
    procedure ReadData(Stream: TStream);
    procedure WriteData(Stream: TStream);
  public
    constructor CreateInitialized(AOwner: TPersistent; const AValue: TGSVector;
      const AStyle: TGSCoordinatesStyle = CsUnknown);
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    procedure WriteToFiler(Writer: TWriter);
    procedure ReadFromFiler(Reader: TReader);
    procedure Initialize(const Value: TGSVector);
    ...
    (* Identifies the coordinates styles.
      ...
      It is used by the TGSCustomCoordinates for internal "assertion" checks
      to detect "misuses" or "misunderstandings" of what the homogeneous
      coordinates system implies. *)
    property Style: TGSCoordinatesStyle read FStyle write FStyle;
    procedure Translate(const TranslationVector: TGSVector); overload;
    procedure Translate(const TranslationVector: TAffineVector); overload;
    procedure AddScaledVector(const Factor: Single; const TranslationVector: TGSVector); overload;
    ...
    procedure Rotate(const AnAxis: TGSVector; AnAngle: Single); overload;
    ...
    function Equals(const AVector: TGSVector): Boolean; reintroduce;
    ...
    property DirectVector: TGSVector read FCoords write SetDirectVector;
  ...
  // A TGSCustomCoordinates that publishes X, Y properties.
  TGSCoordinates2 = class(TGSCustomCoordinates)
  published
    property X stored False;
    property Y stored False;
  end;

  // A TGSCustomCoordinates that publishes X, Y, Z properties.
  TGSCoordinates3 = class(TGSCustomCoordinates)
  published
    property X stored False;
    property Y stored False;
  ...
  // A TGSCustomCoordinates that publishes X, Y, Z, W properties.
  TGSCoordinates4 = class(TGSCustomCoordinates)
  published
    property X stored False;
    property Y stored False;
    property Z stored False;
  ...
  TGSCoordinates = TGSCoordinates3;

  (* Actually Sender should be TGSCustomCoordinates, but that would require
     changes in a some other GLScene units and some other projects that use
     TGSCoordinatesUpdateAbleComponent *)
  IGSCoordinatesUpdateAble = interface(IInterface)
    ['{ACB98D20-8905-43A7-AFA5-225CF5FA6FF5}']
    procedure CoordinateChanged(Sender: TGSCustomCoordinates);
  end;

  TGSCoordinatesUpdateAbleComponent = class(TGSUpdateAbleComponent, IGSCoordinatesUpdateAble)
  public
    procedure CoordinateChanged(Sender: TGSCustomCoordinates); virtual; abstract;
  end;
...
var
  (* Specifies if TGSCustomCoordinates should allocate memory for
...
// ------------------
// ------------------ TGSCustomCoordinates ------------------
// ------------------
constructor TGSCustomCoordinates.CreateInitialized(AOwner: TPersistent;
  const AValue: TGSVector; const AStyle: TGSCoordinatesStyle = CsUnknown);
begin
  Create(AOwner);
  Initialize(AValue);
  FStyle := AStyle;
...
//---------------------------------------------------------------------------
destructor TGSCustomCoordinates.Destroy;
begin
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.Initialize(const Value: TGSVector);
begin
  FCoords := Value;
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.Assign(Source: TPersistent);
begin
  if Source is TGSCustomCoordinates then
    FCoords := TGSCustomCoordinates(Source).FCoords
  else
    inherited;
end;

//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.WriteToFiler(Writer: TWriter);
var
  WriteCoords: Boolean;
begin
  with Writer do
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.ReadFromFiler(Reader: TReader);
var
  N: Integer;
begin
  with Reader do
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.DefineProperties(Filer: TFiler);
begin
  inherited;
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.ReadData(Stream: TStream);
begin
  Stream.Read(FCoords, SizeOf(FCoords));
end;

//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.WriteData(Stream: TStream);
begin
  Stream.Write(FCoords, SizeOf(FCoords));
end;

//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.NotifyChange(Sender: TObject);
var
  Int: IGSCoordinatesUpdateAble;
begin
  if Supports(Owner, IGSCoordinatesUpdateAble, Int) then
    Int.CoordinateChanged(TGSCoordinates(Self));
  inherited NotifyChange(Sender);
end;

//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.Translate(const TranslationVector: TGSVector);
begin
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.Translate(const TranslationVector
  : TAffineVector);
begin
  FCoords.X := FCoords.X + TranslationVector.X;
  FCoords.Y := FCoords.Y + TranslationVector.Y;
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.AddScaledVector(const Factor: Single;
  const TranslationVector: TGSVector);
var
  F: Single;
begin
  F := Factor;
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.Rotate(const AnAxis: TAffineVector;
  AnAngle: Single);
begin
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.Rotate(const AnAxis: TGSVector; AnAngle: Single);
begin
  RotateVector(FCoords, AnAxis, AnAngle);
  NotifyChange(Self);
end;
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.Normalize;
begin
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.Invert;
begin
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.Scale(Factor: Single);
begin
...
//---------------------------------------------------------------------------
function TGSCustomCoordinates.VectorLength: Single;
begin
  Result := Stage.VectorGeometry.VectorLength(FCoords);
end;

//---------------------------------------------------------------------------
function TGSCustomCoordinates.VectorNorm: Single;
begin
  Result := Stage.VectorGeometry.VectorNorm(FCoords);
end;

//---------------------------------------------------------------------------
function TGSCustomCoordinates.MaxXYZ: Single;
begin
  Result := MaxXYZComponent(FCoords);
end;

//---------------------------------------------------------------------------
function TGSCustomCoordinates.Equals(const AVector: TGSVector): Boolean;
begin
  Result := VectorEquals(FCoords, AVector);
end;

//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetVector(const X, Y: Single; Z: Single = 0);
begin
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetVector(const V: TAffineVector);
begin
  Assert(FStyle = csVector, csVectorHelp);
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetVector(const V: TGSVector);
begin
  Assert(FStyle = csVector, csVectorHelp);
  Stage.VectorGeometry.SetVector(FCoords, V);
  NotifyChange(Self);
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetVector(const X, Y, Z, W: Single);
begin
  Assert(FStyle = csVector, csVectorHelp);
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetDirectCoordinate(const Index: Integer;
  const AValue: Single);
begin
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetDirectVector(const V: TGSVector);
begin
  FCoords.X := V.X;
  FCoords.Y := V.Y;
  FCoords.Z := V.Z;
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetToZero;
begin
  FCoords.X := 0;
  FCoords.Y := 0;
  FCoords.Z := 0;
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetPoint(const X, Y, Z: Single);
begin
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetPoint(const V: TAffineVector);
begin
  Assert(FStyle = CsPoint, CsPointHelp);
  MakePoint(FCoords, V);
  NotifyChange(Self);
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetPoint(const V: TGSVector);
begin
  Assert(FStyle = CsPoint, CsPointHelp);
  MakePoint(FCoords, V);
  NotifyChange(Self);
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetPoint2D(const X, Y: Single);
begin
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetPoint2D(const Vector: TAffineVector);
begin
  Assert(FStyle = CsPoint2D, CsPoint2DHelp);
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetPoint2D(const Vector: TGSVector);
begin
  Assert(FStyle = CsPoint2D, CsPoint2DHelp);
  MakeVector(FCoords, Vector);
  NotifyChange(Self);
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetPoint2D(const Vector: TVector2f);
begin
  Assert(FStyle = CsPoint2D, CsPoint2DHelp);
...
//---------------------------------------------------------------------------
function TGSCustomCoordinates.AsAddress: PSingle;
begin
  Result := @FCoords;
end;

//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetAsVector(const Value: TGSVector);
begin
  FCoords := Value;
  case FStyle of
    CsPoint2D:
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetAsAffineVector(const Value: TAffineVector);
begin
  case FStyle of
    CsPoint2D:
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetAsPoint2D(const Value: TVector2f);
begin
  case FStyle of
...
//---------------------------------------------------------------------------
function TGSCustomCoordinates.GetAsAffineVector: TAffineVector;
begin
  Stage.VectorGeometry.SetVector(Result, FCoords);
end;

//---------------------------------------------------------------------------
function TGSCustomCoordinates.GetAsPoint2D: TVector2f;
begin
  Result.X := FCoords.X;
  Result.Y := FCoords.Y;
end;
...
//---------------------------------------------------------------------------
procedure TGSCustomCoordinates.SetCoordinate(const AIndex: Integer;
  const AValue: Single);
begin
...
//---------------------------------------------------------------------------
function TGSCustomCoordinates.GetCoordinate(const AIndex: Integer): Single;
begin
  Result := FCoords.V[AIndex];
end;

//---------------------------------------------------------------------------
function TGSCustomCoordinates.GetDirectCoordinate(
...
//---------------------------------------------------------------------------
function TGSCustomCoordinates.GetAsString: String;
begin
  case Style of
    CsPoint2D:
...
RegisterClasses([TGSCoordinates2, TGSCoordinates3, TGSCoordinates4]);

finalization //==============================================================

// UnRegisterClasses([TGSCoordinates2, TGSCoordinates3, TGSCoordinates4]);
