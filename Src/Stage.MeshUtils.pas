...
*)
procedure ConvertStripToList(const strip: TGSAffineVectorList;
  list: TGSAffineVectorList); overload;
procedure ConvertStripToList(const strip: TGSIntegerList;
  list: TGSIntegerList); overload;
procedure ConvertStripToList(const strip: TGSAffineVectorList;
  const indices: TGSIntegerList; list: TGSAffineVectorList); overload;
function ConvertStripToList(const AindicesList: PLongWordArray; Count: LongWord;
  RestartIndex: LongWord): TGSLongWordList; overload;
function ConvertFansToList(const AindicesList: PLongWordArray; Count: LongWord;
  RestartIndex: LongWord): TGSLongWordList;
// Expands an indexed structure into a non-indexed structure.
procedure ConvertIndexedListToList(const data: TGSAffineVectorList;
  const indices: TGSIntegerList; list: TGSAffineVectorList);
(* 
...
*)
function BuildVectorCountOptimizedIndices(const vertices: TGSAffineVectorList;
  const normals: TGSAffineVectorList = nil;
  const texCoords: TGSAffineVectorList = nil): TGSIntegerList;
(* 
...
*)
procedure RemapReferences(reference: TGSAffineVectorList;
  const indices: TGSIntegerList); overload;
procedure RemapReferences(reference: TGSIntegerList;
  const indices: TGSIntegerList); overload;
(* 
  ...
  coherent. 
*)
procedure RemapAndCleanupReferences(reference: TGSAffineVectorList;
...
*)
function RemapIndicesToIndicesMap(remapIndices: TGSIntegerList): TGSIntegerList;
(* 
...
*)
procedure RemapTrianglesIndices(indices, indicesMap: TGSIntegerList);
(* 
  Remaps a list of indices.
  The indicesMap provides newVertexIndex:=indicesMap[oldVertexIndex] 
*)
procedure remapIndices(indices, indicesMap: TGSIntegerList);
(* 
  ...
  submeshes etc.) 
*)
procedure UnifyTrianglesWinding(indices: TGSIntegerList);
// Inverts the triangles winding (vertex order).
procedure InvertTrianglesWinding(indices: TGSIntegerList);
(* 
...
*)
function BuildNormals(reference: TGSAffineVectorList; indices: TGSIntegerList)
  : TGSAffineVectorList;
(* 
...
*)
function BuildNonOrientedEdgesList(triangleIndices: TGSIntegerList;
  triangleEdges: TGSIntegerList = nil; edgesTriangles: TGSIntegerList = nil)
  : TGSIntegerList;
(* 
...
*)
procedure WeldVertices(vertices: TGSAffineVectorList; indicesMap: TGSIntegerList;
  weldRadius: Single);
(* 
  ...
  The function returns a list of TGSIntegerList, each of these lists hosting
...
*)
function StripifyMesh(indices: TGSIntegerList; maxVertexIndex: Integer;
  agglomerateLoneTriangles: Boolean = False): TGSPersistentObjectList;
(* 
...
*)
procedure IncreaseCoherency(indices: TGSIntegerList; cacheSize: Integer);
...
*)
procedure SubdivideTriangles(smoothFactor: Single; vertices: TGSAffineVectorList;
  triangleIndices: TGSIntegerList; normals: TGSAffineVectorList = nil;
...
function MakeTriangleAdjacencyList(const AindicesList: PLongWordArray;
  Count: LongWord; const AVerticesList: PAffineVectorArray): TGSLongWordList;
...
//---------------------------------------------------------------------------
procedure ConvertStripToList(const strip: TGSIntegerList; list: TGSIntegerList);
var
  i: Integer;
...
var
  ...
  found: Boolean;
  hashSize: Integer;
  hashTable: array of TGSIntegerList;
  list: TGSIntegerList;
...
begin
  ...
  begin
    hashTable[i] := TGSIntegerList.Create;
...
//---------------------------------------------------------------------------
procedure RemapReferences(reference: TGSIntegerList; const indices: TGSIntegerList);
var
  i, n: Integer;
  tag: array of Byte;
...
var
  ...
  triangleStack: TGSIntegerList; // marks triangles winded, that must be processed
...
begin
  ...
  triangleStack := TGSIntegerList.Create;
  try
...
var
  edgesHash: array [0 .. cEdgesHashMax] of TGSIntegerList;
  curTri: Integer;
  edges: TGSIntegerList;
...
begin
  ...
  begin
    edgesHash[j] := TGSIntegerList.Create;
...
var
  ...
  trisOfVertex: array of TGSIntegerList;
  candidates: TGSIntegerList;
  indicesList: PIntegerArray;
begin
  ...
  for i := 0 to High(trisOfVertex) do
    trisOfVertex[i] := TGSIntegerList.Create;
  candidates := TGSIntegerList.Create;
...
var
  ...
  vertexTris: array of TGSIntegerList;
  indicesList: PIntegerArray;
  indicesCount: Integer;
  currentStrip: TGSIntegerList;
...
var
  ...
  loneTriangles: TGSIntegerList;
begin
  Assert((indices.Count mod 3) = 0, 'indices count is not a multiple of 3!');
  Result := TGSPersistentObjectList.Create;
  ...
  for i := 0 to High(vertexTris) do
    vertexTris[i] := TGSIntegerList.Create;
  n := 0;
  triangle := 0;
  ...
  begin
    loneTriangles := TGSIntegerList.Create;
  ...
  begin
    ...
    begin
      ...
      begin
        currentStrip := TGSIntegerList.Create;
...
var
  i, a, b, c, nv: Integer;
  edges: TGSIntegerList;
  triangleEdges: TGSIntegerList;
  p, n: TAffineVector;
  f: Single;
begin
  // build edges list
  triangleEdges := TGSIntegerList.Create;
  try
...
begin
  ...
  boundaryList[b0].maxSqArea := Stage.MeshUtils.polygonArea(b0);
...
begin
  ...
  begin
    ...
    boundaryList[0].maxSqArea := Stage.MeshUtils.polygonArea(0);
    maxMaxSqArea := boundaryList[0].maxSqArea;

    for i := 1 to Count - 1 do
    begin
      boundaryList[i].maxSqArea := Stage.MeshUtils.polygonArea(i);
...
begin
  ...
  NewIndices := TGSLongWordList.Create;
