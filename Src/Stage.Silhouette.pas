...
type
  ...
  (* Base class storing a volume silhouette.
    ...
    This is the base class, you can use the TGSSilhouette subclass if you
    need some helper methods for generating the indexed sets. *)
  TGSSilhouette = class
  private
    FVertices: TGSVectorList;
    FIndices: TGSIntegerList;
    FCapIndices: TGSIntegerList;
    FParameters: TGSSilhouetteParameters;
  protected
    procedure SetIndices(const value: TGSIntegerList);
    procedure SetCapIndices(const value: TGSIntegerList);
    procedure SetVertices(const value: TGSVectorList);
  public
    constructor Create; virtual;
    destructor Destroy; override;
    property Parameters: TGSSilhouetteParameters read FParameters write FParameters;
    property Vertices: TGSVectorList read FVertices write SetVertices;
    property Indices: TGSIntegerList read FIndices write SetIndices;
    property CapIndices: TGSIntegerList read FCapIndices write SetCapIndices;
  ...
  public
    ...
    procedure CreateSilhouette(const ASilhouetteParameters: TGSSilhouetteParameters; var ASilhouette: TGSSilhouette;
      AddToSilhouette: Boolean); virtual;
    constructor Create(APrecomputeFaceNormal: Boolean); virtual;
  end;

  TGSConnectivity = class(TGSBaseConnectivity)
  protected
    ...
    FEdgeVertices: TGSIntegerList;
    FEdgeFaces: TGSIntegerList;
    FFaceVisible: TGSByteList;
    FFaceVertexIndex: TGSIntegerList;
    FFaceNormal: TGSAffineVectorList;
    FVertexMemory: TGSIntegerList;
    FVertices: TGSAffineVectorList;
    function GetEdgeCount: integer; override;
    function GetFaceCount: integer; override;
    function ReuseOrFindVertexID(const SeenFrom: TAffineVector; ASilhouette: TGSSilhouette; index: integer): integer;
  public
...
// ------------------ TGSSilhouette ------------------
// ------------------
constructor TGSSilhouette.Create;
begin
  inherited;
  FVertices := TGSVectorList.Create;
  FIndices := TGSIntegerList.Create;
  FCapIndices := TGSIntegerList.Create;
end;

//---------------------------------------------------------------------------
destructor TGSSilhouette.Destroy;
begin
  FCapIndices.Free;
  FIndices.Free;
  FVertices.Free;
...
//---------------------------------------------------------------------------
procedure TGSSilhouette.SetIndices(const value: TGSIntegerList);
begin
  FIndices.Assign(value);
end;

//---------------------------------------------------------------------------
procedure TGSSilhouette.SetCapIndices(const value: TGSIntegerList);
begin
  FCapIndices.Assign(value);
end;

//---------------------------------------------------------------------------
procedure TGSSilhouette.SetVertices(const value: TGSVectorList);
begin
  FVertices.Assign(value);
end;

//---------------------------------------------------------------------------
procedure TGSSilhouette.Flush;
begin
  FVertices.Flush;
  FIndices.Flush;
  FCapIndices.Flush;
...
//---------------------------------------------------------------------------
procedure TGSSilhouette.ExtrudeVerticesToInfinity(const origin: TAffineVector);
var
...
// ------------------ TGSSilhouette ------------------
// ------------------
procedure TGSSilhouette.AddEdgeToSilhouette(const v0, v1: TAffineVector; tightButSlow: Boolean);
begin
...
//---------------------------------------------------------------------------
procedure TGSSilhouette.AddIndexedEdgeToSilhouette(const Vi0, Vi1: integer);
...
//---------------------------------------------------------------------------
procedure TGSSilhouette.AddCapToSilhouette(const v0, v1, v2: TAffineVector; tightButSlow: Boolean);
begin
  if tightButSlow then
...
//---------------------------------------------------------------------------
procedure TGSSilhouette.AddIndexedCapToSilhouette(const Vi0, Vi1, vi2: integer);
begin
...
// ------------------ TGSBaseConnectivity ------------------
// ------------------
constructor TGSBaseConnectivity.Create(APrecomputeFaceNormal: Boolean);
begin
  FPrecomputeFaceNormal := APrecomputeFaceNormal;
end;

//---------------------------------------------------------------------------
procedure TGSBaseConnectivity.CreateSilhouette(const ASilhouetteParameters: TGSSilhouetteParameters;
...
// ------------------ TGSConnectivity ------------------
// ------------------
function TGSBaseConnectivity.GetEdgeCount: integer;
begin
  result := 0;
end;

//---------------------------------------------------------------------------
function TGSBaseConnectivity.GetFaceCount: integer;
begin
  result := 0;
end;

//---------------------------------------------------------------------------
constructor TGSConnectivity.Create(APrecomputeFaceNormal: Boolean);
begin
  FFaceVisible := TGSByteList.Create;
  FFaceVertexIndex := TGSIntegerList.Create;
  FFaceNormal := TGSAffineVectorList.Create;
  FEdgeVertices := TGSIntegerList.Create;
  FEdgeFaces := TGSIntegerList.Create;
  FPrecomputeFaceNormal := APrecomputeFaceNormal;
  FVertexMemory := TGSIntegerList.Create;
  FVertices := TGSAffineVectorList.Create;
end;

//---------------------------------------------------------------------------
destructor TGSConnectivity.Destroy;
begin
  Clear;
  FFaceVisible.Free;
...
//---------------------------------------------------------------------------
procedure TGSConnectivity.Clear;
begin
  FEdgeVertices.Clear;
  FEdgeFaces.Clear;
  FFaceVisible.Clear;
...
//---------------------------------------------------------------------------
procedure TGSConnectivity.CreateSilhouette(const silhouetteParameters: TGSSilhouetteParameters; var ASilhouette: TGSSilhouette;
...
//---------------------------------------------------------------------------
function TGSConnectivity.GetEdgeCount: integer;
begin
  result := FEdgeVertices.Count div 2;
end;

//---------------------------------------------------------------------------
function TGSConnectivity.GetFaceCount: integer;
begin
  result := FFaceVisible.Count;
end;

//---------------------------------------------------------------------------
function TGSConnectivity.ReuseOrFindVertexID(const SeenFrom: TAffineVector; ASilhouette: TGSSilhouette; index: integer): integer;
var
  pMemIndex: PInteger;
  memIndex, i: integer;
  oldCount: integer;
...
//---------------------------------------------------------------------------
function TGSConnectivity.AddIndexedEdge(vertexIndex0, vertexIndex1: integer; FaceID: integer): integer;
var
  i: integer;
...
//---------------------------------------------------------------------------
function TGSConnectivity.AddIndexedFace(Vi0, Vi1, vi2: integer): integer;
var
  FaceID: integer;
begin
...
//---------------------------------------------------------------------------
function TGSConnectivity.AddFace(const vertex0, vertex1, vertex2: TAffineVector): integer;
var
  Vi0, Vi1, vi2: integer;
begin
...
//---------------------------------------------------------------------------
function TGSConnectivity.AddQuad(const vertex0, vertex1, vertex2, vertex3: TAffineVector): integer;
var
