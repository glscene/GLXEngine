...
var
  FormPhysXScatter: TFormPhysXScatter;
  Plane: dword;
...
begin
  ...
  FormPhysXScatter.Caption := 'PhysX SDK version ' + ByteToStr(dMaj) + '.' +
...
procedure TFormPhysXScatter.CadencerProgress(Sender: TObject;
  const deltaTime, newTime: Double);
var
  i: Integer;
begin
...
procedure TFormPhysXScatter.FormCreate(Sender: TObject);
var
  i: Integer;
begin
...
procedure TFormPhysXScatter.FormDestroy(Sender: TObject);
begin
  DemoFree;
end;

procedure TFormPhysXScatter.TimerTimer(Sender: TObject);
begin
  Edit1.Text := Format('%.1f FPS', [GLSV.FramesPerSecond]);
  GLSV.ResetPerformanceMonitor;
end;

procedure TFormPhysXScatter.GLSVMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if Shift <> [] then
  begin
