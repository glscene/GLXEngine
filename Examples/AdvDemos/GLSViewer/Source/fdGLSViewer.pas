...
begin
  ...
  begin
    DataModuleImages.MaterialLib.LoadFromFile(dmDialogs.ODTextures.FileName);
    for I := 0 to DataModuleImages.MaterialLib.Materials.Count - 1 do
      with DataModuleImages.MaterialLib.Materials[I].Material do
...
begin
  ...
  begin
    with DataModuleImages.MaterialLib.Materials do
    begin
...
begin
  if dmDialogs.SDTextures.Execute then
    DataModuleImages.MaterialLib.SaveToFile(dmDialogs.SDTextures.FileName);
end;
...
begin
  if DataModuleImages.MaterialLib.Materials.Count = 0 then
  begin
    ffObject.Material.MaterialLibrary := DataModuleImages.MaterialLib;
    LibMat := DataModuleImages.MaterialLib.Materials.Add;
...
begin
  ...
  DataModuleImages.MaterialLib.Materials.Clear;
  ffObject.MeshObjects.Clear;
  ffObject.LoadFromFile(FileName);
  SetupFreeFormShading;
  acFileSaveTextures.Enabled := (DataModuleImages.MaterialLib.Materials.Count > 0);
  acFileOpenTexLib.Enabled := (DataModuleImages.MaterialLib.Materials.Count > 0);
...
var
  ...
  mo: TGLMeshObject;
  fg: TFGVertexIndexList;
  strips: TGSPersistentObjectList;
begin
  ...
  try
    ...
    begin
      ...
      fg.VertexIndices := (strips[I] as TGSIntegerList);
      if I = 0 then
        fg.Mode := fgmmTriangles
      else
...
begin
  ...
  case tvScene.Selected.SelectedIndex of
    ...
    41: //Atmosphere
    begin
      Sphere := TGLSphere.CreateAsChild(dcWorld);
      // Sphere.Material.FrontProperties.Diffuse.Color := clrYellow;
      Sphere.Material.FrontProperties.Diffuse.RandomColor();
      Atmosphere := TGLAtmosphere.CreateAsChild(Sphere);
      Atmosphere.Sun := GLLensFlare1;
      Atmosphere.SetOptimalAtmosphere(Sphere.Radius);
    end;
    42: //SkyBox
    begin
      SkyBox.Visible := True;
    ...
    begin
      ...
      DataModuleImages.MLTree.AddTextureMaterial('TreeBark', 'zbark_016.jpg').Material.Texture.TextureMode := tmModulate;
      DataModuleImages.MLTree.AddTextureMaterial('LeafTexture', 'leaf.tga').Material.Texture.TextureMode := tmModulate;
      DataModuleImages.MLTree.AddTextureMaterial('FrutTexture', 'maple_multi.tga').Material.Texture.TextureMode := tmModulate;
    end;
    else
     begin
       SkyBox.Visible := False;
       SkyDome.Visible := False;
       EarthSkyDome.Visible := False;
     end;
  end;
end;
